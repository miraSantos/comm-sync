...
load("data/2023_Jul_10_dfcount_index.RData")
...
temp = as.matrix(cbind(time_index,dfcarbon_conc["solar"]))
dat = as.matrix(cbind(time_index,(dfcarbon_conc[shared_list[i]])^(1/3)))
res = wtc_arc(d1=dat,d2 =temp,
              ...
              anrands=600,
              quiet=TRUE)
save(res,file=paste0("/vortexfs1/scratch/msantos/comm_sync/results/solar_wtc_arc/solar_wtc_arc_",shared_list[i],".RData"))
plot_wtc_arc(dfcarbon_conc,res,title=paste0("Coherence between Solar Irradiance and ",shared_list[i]),
                   save_folder="/vortexfs1/scratch/msantos/comm_sync/results/solar/",
                   save_name = paste0("coherence_solar_phase_",shared_list[i],".png"),
                   plot.phase=TRUE)
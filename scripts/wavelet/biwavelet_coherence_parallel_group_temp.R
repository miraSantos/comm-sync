...
load("data/2023_Jul_10_dfcarbon_group.RData")
...
temp = as.matrix(cbind(time_index,dfcarbon_group["temp_beam"]))
dat = as.matrix(cbind(time_index,(dfcarbon_group[group_list[i]])^(1/3)))
res = wtc_arc(d1=dat,d2 =temp,
              ...
              anrands=600,
              quiet=TRUE)
save(res,file=paste0("/vortexfs1/scratch/msantos/comm_sync/results/temp_wtc_arc_",group_list[i],".RData"))
plot_wtc_arc(dfcarbon_group,res,title=paste0("Coherence between Temperature and ",group_list[i]),
             save_folder="/vortexfs1/scratch/msantos/comm_sync/results/",

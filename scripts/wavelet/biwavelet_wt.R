...
dat = as.matrix(cbind(time_index,scale(dfcarbon_group$Dinoflagellate)))
res = wt_arc(dat,
             s0=16,
             max.scale = 365*4,
             asig.level = c(0.95),
...
plot_single_wt_arc(dfcarbon_group,
                   res,
                   title="Wavelet Transform of Dinoflagellate from 2006-2022",
...
dat = as.matrix(cbind(time_index,scale(df_sal_filled$salt)))
res=wt_arc(dat)
save(res,file=paste0(basepath,"/data/res_2023_oct_6.RData"))
load(paste0(basepath,"/data/res_2023_oct_6.RData"))
plot.biwavelet_adv(res)

...
plot.wt.simulation <- function(res,sim_x,sim_y,save_folder,save_name){
  x = res
ncol = 64
fill.cols = NULL
xlab = "Time"
...
png(filename=paste0(save_folder,save_name),
    width=1000,height=1000,res=120, units = "px")

par(mfrow=(c(2,1)))#bottom,left,top,right
plot(sim_x,sim_y,type="l",xlab="Time",ylab="Abundance",xaxt="n")
axis(1, at = seq(0, length(sim_x), by = 500),las=3)
image.plot(x$t,
           yvals,
           t(zvals),
           zlim = zlim,

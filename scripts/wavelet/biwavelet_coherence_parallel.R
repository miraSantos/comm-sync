...
load("data/2023_Jul_19_dfcount_index.RData")
...
env= as.matrix(cbind(time_index,dfcarbon_conc["temp_beam"]))
...
res = wtc_arc(d1=dat,d2 =env,
              ...
              anrands=600,
              quiet=TRUE)
save(res,file=paste0("/vortexfs1/scratch/msantos/comm_sync/results/temp_wtc_arc/temp_wtc_arc_",shared_list[i],".RData"))
plot_wtc_arc(dfcarbon_conc,res,title=paste0("Coherence between Temperature and ",shared_list[i]),
                   save_folder="/vortexfs1/scratch/msantos/comm_sync/results/temp/",

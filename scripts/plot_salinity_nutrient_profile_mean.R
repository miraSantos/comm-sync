...
dfj %>% filter(season=="Summer",STRATA %in% strata_index,
  ...
  facet_grid(cols=vars(regime))+
  scale_colour_discrete(name = "Profile ID")+
  xlab("Depth (m)") + ylab("Temperature (Deg C)")+
  guides(color = FALSE, size = FALSE)
...
ggsave(filename=paste0(basepath,"/figures/environmental/nutrients/nitrite_nitrate/nitrate_mean_regime_",strata_name,".png"),
...
df_temp <- temp %>% group_by(profile_id) %>% mutate(nut_max = max(nitrite_nitrate))  %>% filter(depth_sampling >= 20,depth_sampling <= 40,nitrite_nitrate >=0.5) %>% mutate(nit_sum_deep =mean(nitrite_nitrate,na.rm=T))

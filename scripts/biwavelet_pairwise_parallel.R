...
print(args[1])
pw_set = combn(shared_list,2)
i = as.numeric(args[1])
print(pw_set[i])

time_index = seq(1,nrow(dfcarbon_conc),1)
dat1_raw = scale(dfcarbon_conc[pw_set[1,i]])
dat2_raw = scale(dfcarbon_conc[pw_set[2,i]])
...
res = wtc_arc(d1=dat1,d2=dat2,
              ...
              anrands=600,
              quiet=TRUE)
save(res,file=paste0("/vortexfs1/scratch/msantos/comm_sync/results/pairwise/pairwise_wtc_arc_",pw_set[1,i],"_",pw_set[2,i],".RData"))
plot_wtc_arc(dfcarbon_conc,res,title=paste0("Coherence between ",pw_set[1,i]," and ",pw_set[2,i]),
             save_folder="/vortexfs1/scratch/msantos/comm_sync/results/pairwise/",
             save_name = paste0("coherence_",pw_set[1,i],"_",pw_set[1,i],".png"),
             plot.phase=TRUE)
